...
class mainWindow(QWidget):
    ...
    def detect_and_classify(yolo_model, image_folder, output_folder):
    ...
    def classify_with_cnn(BP, cut_folder):
        confidences_list = []

        # ✅ 讀取裁切資料夾的所有圖片
        cut_images = [f for f in os.listdir(cut_folder) if f.endswith('.jpg')]

        for img_file in cut_images:
            img_path = os.path.join(cut_folder, img_file)
            ...
            with torch.no_grad():
                ...
                confidence, _ = torch.max(prediction, 1)
...
if __name__ == '__main__':
    ...
    yolo_model, BP = main_window.load_model()

    # 设置输入与输出路径
    image_folder = 'F'
    output_folder = 'CutTest/bad'
    
    result = main_window.classify_with_cnn(yolo_model,BP,image_folder,output_folder)
    
    detect_and_cut(yolo_model, image_folder, cut_folder)
    
    confidences = classify_with_cnn(BP, cut_folder)
